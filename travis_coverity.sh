...
export MOVEIT_CI_DIR="$PWD/.moveit_ci"  # path to the directory running the current script
...
function build_workspace() {
   ...
   ARCH=`arch`
   ln -v -s $TOOL_DIR/bin/libcapture-linux64-$ARCH.so $TOOL_DIR/bin/libcapture-linux64-\\\${PLATFORM}.so
